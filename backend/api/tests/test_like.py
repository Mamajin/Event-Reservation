...
class LikeTestAPI(LikeModelsTest):
    ...
    def test_toggle_like_on_new_event(self):
        ...
        response = self.client.put(f'/{self.event_test.id}/toggle-like', headers={'Authorization': f'Bearer {token}'})
    ...
    def test_toggle_like_on_non_existent_event(self):
        token = self.get_token_for_user(self.test_user)
        response = self.client.put(f'/{999}/toggle-like', headers={'Authorization': f'Bearer {token}'})
