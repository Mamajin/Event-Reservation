    ...
    return (
        <div className="flex bg-white">
            <div className={`bg-dark-purple h-screen p-5 pt-8 ${open ? "w-72" : "w-20"} duration-300 relative`}>
                <ArrowLeftCircleIcon
                    className={`bg-white text-dark-purple w-9 h-9 rounded-full absolute -right-5 top-9 border-dark-purple cursor-pointer ${!open && "rotate-180"}`}
                ...
                <ul className="pt-2">
                    {Menus.map((menu, index) => (
                        <li key={index} className="text-gray-300 text-sm flex items-center gap-x-4 cursor-pointer p-6">
                            <Link to={menu.path} className={`text-base font-medium flex1 ${!open && "hidden"}`}>
                                {menu.title}
                            </Link>
                        </li>
                    ))}
